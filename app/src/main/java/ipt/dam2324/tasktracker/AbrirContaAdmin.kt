...
//Classe responsável pela atividade de criação de conta do admin
class AbrirContaAdmin : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_abrir_conta_admin)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.statusBarColor = ContextCompat.getColor(this, R.color.black)
        }

        val btnCriarContaAdmin = findViewById<Button>(R.id.btnCriarContaAdmin)
        btnCriarContaAdmin.setOnClickListener {
            processarCriarConta()
    ...
    private fun processarCriarConta() {
        val editTextNome = findViewById<EditText>(R.id.editTextNomeAdmin)
        val editTextApelido = findViewById<EditText>(R.id.editTextApelidoAdmin)
        val editTextUsername = findViewById<EditText>(R.id.editTextUsernameAdmin)
        val editTextEmail = findViewById<EditText>(R.id.editTextEmailAdmin)
        val editTextPassword = findViewById<EditText>(R.id.editTextPasswordAdmin)
                ...
                if (response.isSuccessful) {
                    ...
                    if (users != null) {
                        ...
                        if (matchedUser != null) {
                            ...
                            Toast.makeText(this@AbrirContaAdmin, "Username já existente. Insira outro username", Toast.LENGTH_SHORT).show()
                        } else {
                            userNotes(note, user)
                ...
                if (response.isSuccessful) {
                    ...
                    if (response.code() == 200) {
                        if (userResponse != null) {
                            ...
                            Toast.makeText(this@AbrirContaAdmin, "Conta criada com sucesso!", Toast.LENGTH_SHORT).show()
                            abrirMain()
                        }
                    }
                } else {
                    Log.e("Falha na adição de usuário", "Código de resposta: ${response.code()}")
                    Toast.makeText(this@AbrirContaAdmin, "Falha ao criar conta. Tente novamente.", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun abrirMain() {
        val intent = Intent(this, Utilizadores::class.java)
